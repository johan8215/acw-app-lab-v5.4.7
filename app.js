...
// helper
function isEmptyWeek(d){ return !d?.ok || !Array.isArray(d.days) || d.days.length===0; }

// ⬇️ Reemplaza el inicio de loadSchedule(email) por esto:
async function loadSchedule(email) {
  const schedDiv = $("#schedule");
  schedDiv.innerHTML = `<p style="color:#007bff;font-weight:500;">Loading your shift...</p>`;

  try {
    let d = await API.getSchedule(email, 0);

    // 🌙 Grace mode (DST/fin de semana): si la semana actual viene vacía, usa Week -1
    if (isEmptyWeek(d)) {
      const prev = await API.getSchedule(email, -1);
      if (!isEmptyWeek(prev)) {
        d = Object.assign({}, prev, { weekLabel: (d?.weekLabel || prev.weekLabel) + " • fallback" });
        toast("⛑️ Grace mode: mostrando Week -1", "info");
      }
    }
    ...
    // …(lo demás de tu render tal cual)…
    let html = `<table><tr><th>Day</th><th>Shift</th><th>Hours</th></tr>`;
    const todayKey = Today.key;
    d.days.forEach(day=>{
      const isToday = todayKey === day.name.slice(0,3).toLowerCase();
    ...
    html += `</table><p class="total">Total Hours: <b>${Number(d.total||0).toFixed(1)}</b></p>`;
    schedDiv.innerHTML = html;
