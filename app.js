...
<script>
// === ACW Settings Rescue v1 — drop-in safe patch ===
(function(){
  function ensureModal(){
    ...
    if (!m){
      ...
      m.innerHTML = `
        <div class="modal-content glass" style="padding:22px; min-width:300px; position:relative;">
          <button class="close" style="position:absolute; right:10px; top:8px; background:none; border:0; font-size:20px; cursor:pointer;">×</button>
          <h3>⚙️ Settings</h3>
          <div class="settings-section">
            <button id="__acwUpdateBtn">🔄 Check for Updates</button>
            <button onclick="logoutUser()">🚪 Log Out</button>
            <button onclick="openChangePassword()">🔑 Change Password</button>
          </div>
          <p id="installTip" style="font-size:.9em;color:#99bbff;margin-top:8px;"></p>
        </div>`;
      document.body.appendChild(m);
      m.querySelector(".close").onclick = closeSettings;
      m.addEventListener("click", (e)=>{ if(e.target===m) closeSettings(); });
      const up = m.querySelector("#__acwUpdateBtn");
      if (up) up.onclick = (window.refreshApp||(()=>location.reload()));
    }
    return m;
  }
  ...
  function openSettingsFix(){
    const m = ensureModal();
    // Cierra overlays que a veces tapan el modal
    document.getElementById("acwhOverlay")?.remove();
    document.getElementById("directoryWrapper")?.remove();

    // Mostrar por encima de todo
    Object.assign(m.style, { display:"flex", alignItems:"center", justifyContent:"center", zIndex:12000 });
  ...
  // Exporta (fuerza el override)
  window.openSettings = openSettingsFix;
  window.closeSettings = closeSettings;

  // Hookea el botón ⚙️ por si el inline no dispara
  document.addEventListener("DOMContentLoaded", ()=>{
    const gear = document.getElementById("settingsBtn");
    if (gear) gear.onclick = openSettingsFix;
  });
})();
</script>
